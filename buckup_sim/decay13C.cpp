...
double mgs13C; /* Mass of 13C in MeV */
double mgsn; /* Mass of neutron in MeV */
...
struct ssimval {
  ...
  double ex_ene13C; /* Excitation energy of 13C */
...
/* Simulation condition **************************************/
  ...
  /* Section 3: In-flight decay from 13C to 12C+n */
  /* Observables after collision point ********************/
  double snvec[4]; /* 4 momentum of n particles */
  double s13cvec[4];    /* 4 momentum of 13C */
  double sK13C;    /* Energy of alpha particles */
  double sthr13C; /* Polar angle of alpha particles in radian */
  double sphr13C; /* Azimuthal angle of alpha particles in radian*/
  double sth13C;  /* Polar angle of alpha particles in degree */
  double sph13C;  /* Azimuthal ang. of alpha parcticles in degree */

  /* Observables after the target *******************/
  /* a ... alpha just after collision, af ... after the target */
  double s13Cfvec[4]; /* 4 momentum of alpha particles */
                         /*             after collision point */
                         /* [0][] .. 1st alpha emitted into a + 8Be channel */
                         /* [1][] .. 2nd alpha emitted from 8Be */
                         /* [2][] .. 2nd alpha emitted from 8Be */

  double sK313C;    /* Energy of alpha particles */
  double sthr13Cf; /* Polar angle of alpha particles in radian */
  double sphr13Cf; /* Azimuthal angle of alpha particles in radian*/
  double sth13Cf;  /* Polar angle of alpha particles in degree */
  double sph13Cf;  /* Azimuthal ang. of alpha parcticles in degree */
  
  double astr13Cf;  /* In-plane ang. stragg. of alpha particle in target */
  double sthrstr13Cf; /* Polar ang. stragg. of alpha particle in target */
  double sphrstr13Cf; /* Azim. ang. stragg. of alpha particle in target */

  double stote13C;  /* Total energy of 6 alpha particles in the target */
  double stote13Cf;  /* Total energy of 6 alpha particles after the target */
  int sn13C; /* Number of alpha particles which escape from the target */
  ...
  double mvec4He[4];  /* Measured four vector of recoiled 4He */
    ...
    /* Section 7: 13C detection */
  int idp13C; /* Detection flag for particle 3 0..Not detected  1..detected */
  double s13Cvec[2][3]; /* 3-dim momentum of 13C */
  double s13Cpos[3]; /* Position in the detector frame */
  double mK13C;  /* Measured energy of 13C */
  double m13Cpos[2][3]; /* Measured position of 13C */
  double mthr13C;  /* Measured polar angle of 13C in radian */
  double mphr13C;  /* Measured azimuthal angle of 13C in radian */
  double mth13C;  /* Measured polar angle of 13C in degree */
  double mph13C;  /* Measured azimuthal angle of 13C in degree */
  double mthxr13C; /* Measured horizontal angle of 13C in radian */
  double mthyr13C; /* Measured vertical angle of 13C in radian */
  double mthx13C;  /* Measured horizontal angle of 13C in degree */
  double mthy13C;  /* Measured vertical angle of 13C in degree */
  double mex13C;  /* Measured excitation energy of 13C */
  int isx13CSi; /* ID of hit channel of Si (horizontal) */
  int isy13CSi; /* ID of hit channel of Si (vertical) */
  double mvec13C[4];  /* Measured four vector of recoiled 13C */  
...
double decay13C(struct skine *k, struct ssimval *s);
...
int detect13C(struct skine *k, struct ssimval *s);
...
int main(int argc, char *argv[]){
  ...
  ssim.ex_ene[0]=7.25;  /* default values */
  ssim.ex_ene[1]=4.44;
  ssim.ex_ene[2]=0;
  ssim.ex_ene13C=7.65;
  ...
  ssim.dSi=41.15; /* distance between target and Si in mm */
  ...
  ssim.offx1=1;  /* Lateral offset of beam spot in mm */
  ...
  mgs13C=13.0*AMU+3.1250; //13C mass calc.
  mgsn=1.0*AMU+8.0714;
  ...
  // section 5
  tree->Branch("idp13C",&ssim.idp13C,"7_idp13C/I");
  tree->Branch("mK13C",&ssim.mK13C,"7_mK13C/D");
  tree->Branch("mth13C",&ssim.mth13C,"7_mth13C/D");
  tree->Branch("mph13C",&ssim.mph13C,"7_mph13C/D");
  tree->Branch("s13Cpos",ssim.s13Cpos,"7_s13Cpos[3]/D");
  tree->Branch("m13Cpos",ssim.m13Cpos[1],"7_m13Cpos[3]/D");
  tree->Branch("m13CchF",&ssim.isx13CSi,"7_m13CchF/I");
  tree->Branch("m13CchR",&ssim.isy13CSi,"7_m13CchR/I");

  tree->Branch("sK13C",&ssim.sK313C,"5_sK13C/D");
  tree->Branch("sth13C",&ssim.sth13Cf,"5_sth13C/D");
  tree->Branch("sph13C",&ssim.sph13Cf,"5_sph13C/D");
  ...
  for(int iev=0;iev<nev*2;iev++){
    ...
    for(int l=0;l<3;l++){
      ssim.sK13C=0; ssim.sthr13C=0; ssim.sphr13C=0; ssim.sth13C=0; ssim.sph13C=0;
    ...
    ssim.sK313C=0; ssim.sth13Cf=0; ssim.sph13Cf=0;
    ssim.s13Cvec[0][0]=0; ssim.s13Cvec[0][1]=0; ssim.s13Cvec[0][2]=0; 
    ssim.s13Cvec[1][0]=0; ssim.s13Cvec[1][1]=0; ssim.s13Cvec[1][2]=0; 
    ssim.s13Cpos[0]=-100; ssim.s13Cpos[1]=-100; ssim.s13Cpos[2]=-100; 
    ssim.mK13C=0;
    ssim.m13Cpos[0][0]=-100; ssim.m13Cpos[0][1]=-100; ssim.m13Cpos[0][2]=-100; 
    ssim.m13Cpos[1][0]=-100; ssim.m13Cpos[1][1]=-100; ssim.m13Cpos[1][2]=-100; 
    ssim.mthr13C=0; ssim.mphr13C=0; ssim.mth13C=0; ssim.mph13C=0;
    ssim.mthxr13C=0; ssim.mthyr13C=0; ssim.mthx13C=0; ssim.mthy13C=0;
    ssim.isx13CSi=-1; ssim.isy13CSi=-1;
    ssim.mvec13C[0]=0; ssim.mvec13C[1]=0; ssim.mvec13C[2]=0; ssim.mvec13C[3]=0; 
...
double decay13C(struct skine *k, struct ssimval *s){
  ...
  c13vec[0]=s->sK4[1]+mgs13C+s->ex_ene[0];
  tmpmom=sqrt(c13vec[0]*c13vec[0]-(mgs13C+s->ex_ene[0])*(mgs13C+s->ex_ene[0]));
  
  deflect2(v1,v2,s->sthr1[1],s->sphr1[1],k->ang_lb4[0]*D_TO_R,s->sphrcm+180*D_TO_R);
  for(i=0;i<3;i++) c13vec[i+1]=v2[i]*tmpmom; 
  ...
  /* 12C + n  */
  getbeta(c13vec,betag);
  vecadd(betag,betag,betag,0.0,-1.0);
  m0p=sqrt(scapro4(c13vec,c13vec));
  cout << c13vec[0]-mgs13C-s->ex_ene[0] << " " << c13vec[1] << " " << c13vec[2] << " " << c13vec[3] << endl;
  
  if(decay2body(p13Cc,pnc,m0p,mgs[2],mn)<0) {
    printf("Cannot decay to 12C + n channel !!\n");
    return(0);
  }
  cout << p13Cc[0]-mgs[2]  << " " << p13Cc[1] << " " << p13Cc[2] << " " << p13Cc[3] << endl;
  
  lortra(p13Cc,s->s13cvec,betag);
  lortra(pnc,s->snvec,betag);

  cout << betag[0] << " " << betag[1] << " " << betag[2] << endl;
  cout << s->s13cvec[0]-mgs[2] << " " << s->s13cvec[1] << " " << s->s13cvec[2] << " " << s->s13cvec[3] << endl;
  cout << s->snvec[0] - mgsn << " " << s->snvec[1] << " " << endl;
  cout << endl;
  
  // Energy loss in the target
  s->sK13C=s->s13cvec[0]-mgs[2];
  s->stote13C+=s->sK13C;
  s->sth13C=gettheta4(s->s13cvec);
  s->sph13C=getphi4(s->s13cvec);
  s->sthr13C=s->sth13C*D_TO_R;
  s->sphr13C=s->sph13C*D_TO_R;
  
  genrndg(&tmpr1,&tmpr2);
  if(s->s13cvec[3]>0) spath=sdep2/cos(s->sthr13C);
  else {
    //      printf("Alpha emitted in backward direction !!!\n");
    spath=fabs(s->sdep1/cos(s->sthr13C));
  }
  // Anguar straggling 
  s->s13Cfvec[0]=-1000;
  s->s13Cfvec[1]=-1000;
  ...
  s->astr13Cf=tmpr1*astrac(s->sK13C,spath,s->tthick);
  //    s->astr13Cf=0;
  s->sthrstr13Cf=s->astr13Cf*sqrt(2.0);
  s->sphrstr13Cf=drand48()*2.0*M_PI;
  deflect2(v1,v2,s->sthr13C,s->sphr13C,
	   s->sthrstr13Cf,s->sphrstr13Cf);
  s->sth13Cf=gettheta(v2);
  s->sph13Cf=getphi(v2);
  s->sthr13Cf=s->sth13Cf*D_TO_R;
  s->sphr13Cf=s->sph13Cf*D_TO_R;
  
  tmpeloss=elossc(s->sK13C,spath,s->tthick);
  
  // Energy straggling 
  s->sK313C=s->sK13C-tmpeloss+tmpr2*estrac(s->sK13C-tmpeloss,spath,s->tthick);
  if(s->sK313C<0){
    s->sK313C=-10000;
    s->sth13Cf=-10000;
    s->sph13Cf=-10000;
    ...
    s->sn13C++;
    s->stote13Cf+=s->sK313C;
    
    // Reconstruct 4 momentum 
    s->s13Cfvec[0]=s->sK313C+m13C;
    tmpmom=sqrt(s->s13Cfvec[0]*s->s13Cfvec[0]-m13C*m13C);
    for(l=0;l<3;l++) s->s13Cfvec[l+1]=v2[l]*tmpmom; 
...
int detect13C(struct skine *k, struct ssimval *s){
  ...
  int quadrant=-1;
  int i,j;
  genvthph(s->sth13Cf,s->sph13Cf,s->s13Cvec[1]);
  rotvec(s->s13Cvec[1],s->s13Cvec[0],1,-1.0*s->thrSi);

  double spath;
  spath=(s->tthick-s->sdep1)/cos(s->sthr13Cf);
  
  genrndg(&tmpr1,&tmpr2);
  s->mK13C=s->sK313C+tmpr1*s->sigK4-elossc(s->sK313C+tmpr1*s->sigK4,spath,s->tthick);
  if(s->s13Cvec[0][2]>0){
    slatex[1][0]=s->sx1;
    slatex[1][1]=s->sy1;
    slatex[1][2]=0;
    rotvec(slatex[1],slatex[0],1,-1.0*s->thrSi);
    vecadd(s->s13Cvec[0],slatex[0],s->s13Cpos,
           (s->dSi-slatex[0][2])/s->s13Cvec[0][2],1.0);    
    tmpr = pow((pow(s->s13Cpos[0],2)+pow(s->s13Cpos[1],2)),0.5);
    if(s->s13Cpos[0]>0) tmptheta=atan(s->s13Cpos[1]/s->s13Cpos[0])*R_TO_D;
    if(s->s13Cpos[0]<0) tmptheta=atan(s->s13Cpos[1]/s->s13Cpos[0])*R_TO_D+180;
    //    printf("13C:x,y,z,r:%f,%f,%f,%f\n",s->s13Cpos[0],s->s13Cpos[1],s->s13Cpos[2],tmpr);

    if((tmpr < s->diaSi/2) && (tmpr > s->innSi/2)){
      s->idp13C=1;
      if(s->s13Cpos[0]>0 && s->s13Cpos[1]>0){
        quadrant=0;
      }else if(s->s13Cpos[0]<0 && s->s13Cpos[1]>0){
        quadrant=1;
      }else if(s->s13Cpos[0]<0 && s->s13Cpos[1]<0){
        quadrant=2;
      }else if(s->s13Cpos[0]>0 && s->s13Cpos[1]<0){
      ...
      s->isx13CSi=(int)((tmpr-s->innSi/2)/((s->diaSi-s->innSi)/2)*(s->nxSi/4)) + 16*quadrant;//%% 
      if(quadrant==0) s->isy13CSi = (int)(((atan(s->s13Cpos[1]/s->s13Cpos[0])*R_TO_D)/22.5)) +4*0;
      if(quadrant==1) s->isy13CSi = (int)(((atan(s->s13Cpos[1]/s->s13Cpos[0])*R_TO_D)/22.5)-1)+4 +4*1;
      if(quadrant==2) s->isy13CSi = (int)(((atan(s->s13Cpos[1]/s->s13Cpos[0])*R_TO_D)/22.5)) +4*2;
      if(quadrant==3) s->isy13CSi = (int)(((atan(s->s13Cpos[1]/s->s13Cpos[0])*R_TO_D)/22.5)-1)+4 +4*3;
      //	printf("x,y:%d,%d\n",s->isx13CSi,s->isy13CSi);
     
	  
      s->m13Cpos[0][2]=s->s13Cpos[2];
      s->m13Cpos[0][0]=(((s->isx13CSi%16+0.5)*((s->diaSi-s->innSi)/2)/(s->nxSi/4))+s->innSi/2) * cos(((s->isy13CSi+0.5)*22.5)*D_TO_R);
      s->m13Cpos[0][1]=(((s->isx13CSi%16+0.5)*((s->diaSi-s->innSi)/2)/(s->nxSi/4))+s->innSi/2) * sin(((s->isy13CSi+0.5)*22.5)*D_TO_R);
      rotvec(s->m13Cpos[0],s->m13Cpos[1],1,s->thrSi);
      //      printf("posx, posy, posz : %f, %f %f\n",s->m13Cpos[0][0],s->m13Cpos[0][1],s->m13Cpos[0][2]);
      
      double tmppos[3]; 
      tmppos[0]=s->m13Cpos[1][0]-s->offx1;
      tmppos[1]=s->m13Cpos[1][1]-s->offy1;
      tmppos[2]=s->m13Cpos[1][2]-0;

      s->mth13C=gettheta(tmppos);
      s->mph13C=getphi(tmppos);
      s->mthr13C=s->mth13C*D_TO_R;
      s->mphr13C=s->mph13C*D_TO_R;
      //      printf("pos,theta:%f,%f\n",s->m4Hepos[1],s->mth4He);
      
      tmpx=sin(s->mthr13C)*cos(s->mphr13C);
      tmpy=sin(s->mthr13C)*sin(s->mphr13C);
      tmpz=cos(s->mthr13C);
      s->mthxr13C=atan(tmpx/tmpz);
      s->mthyr13C=atan(tmpy/tmpz);
      s->mthx13C=s->mthxr13C*R_TO_D;
      s->mthy13C=s->mthyr13C*R_TO_D;
      //      s->mex13C=calcex4(s->mthr13C,k->m1,k->m2,k->m1,k->m2,s->enepa,s->mK13C); //need correction
      s->mex13C=s->enepa-s->mK13C-s->mK4He;

      s->mvec13C[0]=k->m4+s->mK13C;
      //  /*Opposite direction from 13C*/
      genvthph(-1.0*s->mth13C,s->mph13C,&s->mvec13C[1]);
      vecadd(&s->mvec13C[1],null,&s->mvec13C[1],
	     sqrt(s->mvec13C[0]*s->mvec13C[0]-k->m4*k->m4),1);
    }
    else{
      s->mK13C=0;
      s->idp13C=0;
...
double tmp(double thetacm_i=40., double thetacm_f=75., int n_div=350, int i_div=0){
  ...
  */
  ...
  double x = thetacm_i + (thetacm_f-thetacm_i)/n_div * (double)i_div;
  num = (p0 + p1*x + p2*exp(-pow(x-p3,2)/(2*pow(p4,2))))/100;
