...
int hit_new(int run){
  ...
  TFile *fin =new TFile(Form("../event%d.root",run));
  ...
  for(ULong64_t evtn=0; evtn<N; evtn++){
    ...
    //2hit
    if(count_f!=2 || count_r!=2) continue;
    if(abs(Energy_f[0]-Energy_r[0])>1) continue;
    if(abs(Energy_f[1]-Energy_r[1])>1) continue;
    if(type_f[0]-type_r[0]+type_f[1]-type_r[1]!=-20) continue;
    hit_n=2;
