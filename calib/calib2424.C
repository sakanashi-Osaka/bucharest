...
int calib2424()
{
 TSpectrum *s[peak_n];
  ...
  ofstream ofs("log2424.txt",std::ios::app);

  for(Int_t i=0;i<ch;i++){
    s[i] = new TSpectrum(peak_n);
    h[i] = new TH1F(Form("h_%d",i),Form("h_%d",i),100,500,900);
    tree->Draw(Form("ADC>>h_%d",i),Form("domain==%d",i+64),"",1e9);
    ...
    for(Int_t j=0;j<peak_n;j++){
      f[i][j]=new TF1(Form("f%d_%d",i,j),"gaus",peak[j]-10,peak[j]+10);
      f[i][j]->SetParameters(100,peak[j],5);
    }

    for(Int_t j=0;j<peak_n;j++){
      h[i]->Fit(Form("f%d_%d",i,j),"+","",peak[j]-10,peak[j]+10);
