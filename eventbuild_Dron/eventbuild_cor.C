...
int main(int argc, char *argv[]){
  ...
  event->Branch("Energy2423",Energy2423,"Energy2423[10][16]/F");
  ...
  //  for(ULong64_t evtn=0; evtn<1e8; evtn++){
    ...
    for(int j=0; j<10; j++){
      for(int k=0; k<16; k++){
	ts_diff[j][k]=-1e6; ADC[j][k]=-1; Energy[j][k]=-1; Energy2423[j][k]=-1; Amax[j][k]=-1; Amax_cor[j][k]=-1; front_ex[j][k]=-100;
      }
    }
    board = -1; ch = -1;
	...
	if(tmp_domain<64) Energy2423[board][ch] = tmp_energy;
	if(tmp_domain>63) Energy2423[board][ch] = p0[tmp_domain-64] + tmp_adc*p1[tmp_domain-64];
