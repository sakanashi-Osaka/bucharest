...
int main(int argc, char *argv[]){
  ...
  */
  ...
  ifstream ifs1(Form("calib_Amax/4He/Dron/test%d.txt",atoi(argv[1]))); //for Dron
  for(int i=0; i<16; i++){
    for(int j=0; j<16; j++){
  ...
  //  for(ULong64_t evtn=0; evtn<5e6; evtn++){
    ...
    //    if(3 < tmp_ex && tmp_ex<6){  // selected leading alpha //gamma*.root
      ...
      distance = get_distance(chf4He,chr4He);
      area = get_area(chf4He,chr4He);
	...
	//	continue;
      ...
      //      if(flag_12C) tree->Fill(); //Dron*.root 
      tree->Fill(); //gomi*.root 
    }
  }
