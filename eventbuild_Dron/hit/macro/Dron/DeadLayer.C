void DeadLayer() {
    ...
    TFile *sim_file = TFile::Open("~/exp/Bucharest2022/simulation/rootfile/ene444_dl52_e70.root");
    if (!sim_file || sim_file->IsZombie()) {
        std::cerr << "Error: Could not open rootfile/ene444_dl52_e70.root" << std::endl;
        return;
    }
    ...
    sim_fit->SetParameter(1, 0.0); // 平行移動 (初期値: 0)
    ...
    // 結果を表示
    TCanvas *c1 = new TCanvas("c1", "Fit Simulation to Experimental Data", 600, 600);
