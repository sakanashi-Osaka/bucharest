...
double Lab_to_CM(double labAngle) {
    // データ読み込み
    ifstream infile("log/theta_444.txt");
    if (!infile) {
...
void hist_Dron_dist444() {
  ...
  TFile *file = new TFile("../rootfile/gamma_2297_2306.root");
    ...
    TFile *outputFile = new TFile("gomi_with_errors.root", "RECREATE");
    ...
    for (int chf = 0; chf < 16; ++chf) {
        for (int chr = 0; chr < 16; ++chr) {
            ...
            TH1D *h1 = new TH1D(TString::Format("h1_%d_%d", chf, chr), "Ex4He", 100, 4, 5);
            ...
            double bg_slope = (bg_right - bg_left) / (5. - 4.);
            double bg_intercept = bg_left - bg_slope * 4.;

            int maxBin = h1->GetMaximumBin();
            double maxBinHeight = h1->GetBinContent(maxBin);
            double val_maxBin = 4. + 0.01 * (double)maxBin;

            // フィット関数を作成 (ガウシアン + 線形バックグラウンド)
            TF1 *fitFunc = new TF1("fitFunc", "[0]*exp(-0.5*((x-[1])/[2])**2) + [3]*x + [4]", 4, 5);
            ...
            // 収量の統計誤差を計算
            double yield_error = amplitude > 0 ? sqrt(amplitude) * sqrt(2 * TMath::Pi()) * sigma / (1.0 / 100) /2 : 0;
