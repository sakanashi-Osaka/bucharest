...
double Amax_origin_f[4][N_HIT_MAX]={};
double Amax_origin_r[4][N_HIT_MAX]={};
...
int main(int argc, char *argv[]){
  ...
  float tmp_amax_origin[N_BOARD][N_CH]={};
  ...
  event->SetBranchAddress("Amax",tmp_amax_origin);
  ...
  //  hit->Branch("count_f",count_f,"count_f[4]/I"); //tmp
  //  hit->Branch("count_r",count_r,"count_r[4]/I"); //tmp
  hit->Branch("ch_f",ch_f,"ch_f[4][10]/I");
  hit->Branch("ch_r",ch_r,"ch_r[4][10]/I");
  hit->Branch("domain_f",domain_f,"domain_f[4][10]/I");
  hit->Branch("domain_r",domain_r,"domain_r[4][10]/I");
  ...
  //  hit->Branch("ene_f",ene_f,"ene_f[4][10]/D"); //tmp
  //  hit->Branch("ene_r",ene_r,"ene_r[4][10]/D"); ///tmp
  ...
  hit->Branch("Amax_f",Amax_f,"Amax_f[4][10]/D");
  hit->Branch("Amax_r",Amax_r,"Amax_r[4][10]/D");
  hit->Branch("Amax_origin_f",Amax_origin_f,"Amax_origin_f[4][10]/D");
  hit->Branch("Amax_origin_r",Amax_origin_r,"Amax_origin_r[4][10]/D");
  //  hit->Branch("Amax_f",Amax_f,"Amax_f[4][10]/D");
  ...
  for(ULong64_t evtn=0; evtn<N; evtn++){
	...
	//       	if(tmp_energy[i][j]>1 && tmp_energy[i][j]<30){
	  ...
	  if(abs(type-22)<3){
	    ...
	    amax_origin_f[type-20][count_f[type-20]]=(double)tmp_amax_origin[i][j];
	  ...
	  if(abs(type-32)<3){
	    ...
	    amax_origin_r[type-30][count_r[type-30]]=(double)tmp_amax_origin[i][j];
	...
	if(abs(ene_f[seg][0]+ene_f[seg][1]-ene_r[seg][0]-0.2)<0.5){; //tmp Dron
	  fill_data(seg,0,0,0,0);	  
	...
	if(abs(ene_f[seg][0]+ene_f[seg][1]+ene_f[seg][2]-ene_r[seg][0]-ene_r[seg][1]-0.2)<0.5){
...
void fill_data(int seg, int num_fi, int num_ff, int num_ri, int num_rf){
  ...
  Amax_f[seg][num_ff]=amax_f[seg][num_fi];
  Amax_r[seg][num_rf]=amax_r[seg][num_ri];
  Amax_origin_f[seg][num_ff]=amax_origin_f[seg][num_fi];
  Amax_origin_r[seg][num_rf]=amax_origin_r[seg][num_ri];
  ...
  //  front_ex[seg][num_ff] = get_front_ex(tmp_theta1, Energy_f[seg][num_ff]);
  front_ex[seg][num_ff] = get_front_ex(get_front_theta(ch_f[seg][num_ff]), Energy_f[seg][num_ff]);
